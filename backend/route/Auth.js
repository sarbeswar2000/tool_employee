...
router.post(
  ...
  [
    ...
    body('f_Pwd').isLength({ min: 5 }).withMessage('Password must be at least 5 characters long'),
  ],
  async (req, res) => {
    ...
    try {
      ...
      await newUser.save();
      res.json({ success: true, message: 'Admin registered successfully', user: newUser });
    } catch (error) {
      console.error(error);
...
router.post(
  ...
  [
    ...
    body("course").isArray().withMessage("At least one course must be selected"),
    body("course.*").isIn(["MCA", "BCA", "BSC"]).withMessage("Invalid course selection")
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    if (!req.file) {
      return res.status(400).json({ success: false, message: "Image is required and must be jpg/png" });
    }
